...
import { getCurriculumContext } from "../_shared/curriculumContext.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    // Grounding precedence: a caller-supplied lessonContext wins; otherwise we
    // RETRIEVE the school's uploaded curriculum for this topic/grade and ground on
    // it. Retrieval is strictly best-effort — any failure or no-match leaves us
    // generating generically and NEVER errors.
    let trimmedLessonContext = typeof lessonContext === 'string' ? lessonContext.slice(0, 50000) : '';
    let groundingSource: 'lesson' | 'curriculum' | 'none' = trimmedLessonContext ? 'lesson' : 'none';
    let groundingDoc: string | null = null;

    if (!trimmedLessonContext && topic) {
      try {
        ...
        if (supabaseUrl && anonKey && serviceKey && authHeader.startsWith('Bearer ')) {
          // Derive the caller's school from their JWT (never trust a client-supplied id).
          const caller = createClient(supabaseUrl, anonKey, { global: { headers: { Authorization: authHeader } } });
          const { data: schoolIds } = await caller.rpc('user_school_ids');
          const schoolId = Array.isArray(schoolIds) && schoolIds.length ? (schoolIds[0] as string) : null;
          if (schoolId) {
            const admin = createClient(supabaseUrl, serviceKey);
            const ctx = await getCurriculumContext(admin, { schoolId, topic, subject, gradeLevel, maxChars: 40000 });
            if (ctx) {
              trimmedLessonContext = ctx.text;
              groundingSource = 'curriculum';
              groundingDoc = ctx.documentTitle;
            }
          }
        }
      } catch (e) {
        console.error('[generate-quiz] curriculum retrieval failed; generating generically:', (e as Error).message);
      }
    }
...
Generate questions in this exact JSON format only (no markdown, no extra text):
{
  ...
  "questions": [
    { "question": "Question text", "type": "multiple_choice", "options": ["Option A","Option B","Option C","Option D"], "correct_answer": "Option A", "explanation": "Why this answer is correct" }
  ]
}
For short_answer questions, ALWAYS include a sample correct answer in correct_answer field.
Mix question types: multiple_choice, true_false (options must be ["True","False"]) and short_answer (omit options). Ensure JSON is valid.`;
    ...
    async function callOpenAIWithRetry(retries = 2, delayMs = 1200): Promise<Response> {
      for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        if (resp.status === 429 && attempt < retries) { await new Promise(r => setTimeout(r, delayMs * (attempt + 1))); continue; }
        return resp;
      }
    ...
    return new Response(JSON.stringify({
      ...quizData,
      _grounded: groundingSource !== 'none',
      _groundingSource: groundingSource,
      _groundingChars: trimmedLessonContext.length,
      _groundingDoc: groundingDoc,
