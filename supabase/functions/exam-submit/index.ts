...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await finalizeSession({
      admin, userId, sessionId: session_id, submitted: answers, auto: !!auto, callerIsSystem: false, supabaseUrl, anonKey, authHeader,
    });
...
export async function finalizeSession(opts: {
  ...
  for (const q of questions) {
    ...
    if (q.question_type === 'short_answer' && selected.trim()) {
      // Best-effort semantic check via existing edge function; fall back to exact match.
      try {
  ...
  // Mirror to test_attempts (service role bypasses the new RLS lockdown)
