...
export const ComprehensiveMonitoringDashboard = () => {
  ...
  const [filterBy, setFilterBy] = useState<'all' | 'excellent' | 'good' | 'needs_attention'>('all');
  ...
  const filteredStudents = studentProgress
    .filter(student => {
      ...
      if (filterBy === 'excellent') return matchesSearch && student.completion_percentage >= 80;
      if (filterBy === 'good') return matchesSearch && student.completion_percentage >= 60 && student.completion_percentage < 80;
      if (filterBy === 'needs_attention') return matchesSearch && student.completion_percentage < 60;
  ...
  const handleExportAllData = () => {
    const csvContent = studentProgress.map(student => ({
      ...
      Insights: student.performance_insights.join('; ')
    }));
    ...
    a.href = url;
    a.download = `comprehensive-monitoring-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const handleStudentReport = async (studentId: string, format: 'pdf' | 'csv') => {
    const report = await generateReport(studentId, format);
    if (report) {
      const student = studentProgress.find(s => s.student_id === studentId);
      const blob = new Blob([report], { type: format === 'csv' ? 'text/csv' : 'text/plain' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${student?.student_name || 'student'}-report.${format}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button 
                            size="sm" 
                            variant="outline"
                            onClick={() => handleStudentReport(student.student_id, 'pdf')}
                          >
