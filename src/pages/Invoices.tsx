...
const Invoices = () => {
  ...
  const load = async () => {
    ...
    if (ids.length) {
      ...
      (profs || []).forEach((p: any) => { map[p.id] = p.display_name || 'Student'; });
      setStudents(map);
    }
    setLoading(false);
  };
  ...
  const payInvoice = async (inv: Invoice) => {
    ...
    try {
      ...
      if (data?.url) {
        ...
        toast({ title: 'Payment unavailable', description: data.message });
      }
    } catch (e: any) {
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-2">
              <Receipt className="h-7 w-7 text-primary" /> Invoices & Fees
            </h1>
            <p className="text-muted-foreground">View and pay your school invoices.</p>
          </div>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-10 text-muted-foreground">No invoices on file.</div>
              ) : (
                    ...
                    <div key={inv.id} className="flex items-center justify-between gap-3 p-4 border border-border rounded-lg flex-wrap">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {students[inv.student_id] || 'Student'} · {inv.due_date ? `Due ${new Date(inv.due_date).toLocaleDateString()}` : 'No due date'}
                        </p>
                      ...
                      <div className="text-right">
                        ...
                        {inv.status === 'paid' ? (
                          <p className="text-xs text-green-600 flex items-center gap-1 justify-end">
                            <CheckCircle2 className="h-3 w-3" /> Paid {inv.paid_at ? new Date(inv.paid_at).toLocaleDateString() : ''}
                          </p>
